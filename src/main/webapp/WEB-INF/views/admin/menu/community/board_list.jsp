						...
						<div class="panel-body">
							<table id="data-table-responsive-qq" class="table table-striped table-bordered align-middle">
								<thead>
									<tr>
										<th width="10%">글번호</th>
										<th width="45%" data-orderable="true">제목</th>
										<th width="text-nowrap">작성자닉네임</th>
										<th class="text-nowrap">게시일</th>
										<th class="text-nowrap">조회수</th>
										<th width="5%"></th>
										<th width="5%"></th>
								...
								<tbody>
									<c:forEach var="qvo" items="${qq_list }" varStatus="status">
										<tr>
											...
											<td>${qvo.qna_hit }</td>
										<!--<td><a href="${pageContext.request.contextPath }/admin/community/qnaboard_delete?qna_board_no=${qvo.qna_board_no }" class="btn btn-sm btn-primary w-60px me-1">삭제</a></td>
											<td><a href="#modal-dialog-qq" class="open_modal btn btn-sm btn-white w-60px" data-bs-toggle="modal" 
											data-id="${qvo.qna_board_no}" data-name="${qvo.qna_title}" data-parent="${qvo.qna_content}"
											data-order="${qvo.qna_regdate}">수정</a></td>-->
											<c:choose>
												<c:when test="${qvo.mem_no==1 || qvo.mem_no==2 }">
													<td><a href="${pageContext.request.contextPath }/admin/community/qnaboard_delete?qna_board_no=${qvo.qna_board_no }" class="btn btn-sm btn-secondary w-60px me-1">삭제</a></td>
													<td><a href="#modal-dialog-qq" class="open_modal btn btn-sm btn-white w-60px" data-bs-toggle="modal" 
													data-id="${qvo.qna_board_no}" data-name="${qvo.qna_title}" data-parent="${qvo.qna_content}"
													data-order="<fmt:parseDate value="${qvo.qna_regdate }" var="qna_regdate" pattern="yyyy-MM-dd'T'HH:mm:ss" /><fmt:formatDate value="${qna_regdate }" pattern="yyyy-MM-dd HH:mm:ss" />">수정</a></td>
												</c:when>
												<c:otherwise>
													<td colspan="2" class="text-center"><a href="${pageContext.request.contextPath }/admin/community/qnaboard_detail?qna_board_no=${qvo.qna_board_no}" class="btn btn-sm btn-success w-120px me-1">상세보기</a></td>
												</c:otherwise>
											</c:choose>
										</tr>
									</c:forEach>
			...
			<div class="modal-body">
				<form method="post" id="category_qq" action="${pageContext.request.contextPath }/admin/community/qnaboard_update">
					<div class="row mb-15px">
						<label class="form-label col-form-label col-md-3">글번호</label>
							<div class="col-md-9">
								<input type="text" id="qna_board_no" class="form-control mb-5px" readonly/>
							</div>
					</div>
					<div class="row mb-15px">
						<label class="form-label col-form-label col-md-3">글제목</label>
							<div class="col-md-9">
								<input type="text" id="qna_title" class="form-control mb-5px"/>
							</div>
					</div>
					<div class="row mb-15px">
						<label class="form-label col-form-label col-md-3">내용</label>
							<div class="col-md-9">
							<!--<input type="text" id="n_content" class="form-control mb-5px"/> -->
								<textarea rows="15" cols="4000" id="qna_content" class="form-control"></textarea>
							</div>
					</div>
					<div class="row mb-15px">
						<label class="form-label col-form-label col-md-3">게시일</label>
